Dipcoin Core version *0.15.1* is now available from:

  <https://download.dipcoin.org/dipcoin-0.15.1/>
...
A number of changes to the way Dipcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 drops per kB (0.00001 DIPP). This is to prepare for dropping the minimum transaction fee to 0.1 drops per kB (0.0001 DIPP) in the next release.
